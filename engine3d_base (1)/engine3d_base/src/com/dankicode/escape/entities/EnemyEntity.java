...
import com.dankicode.escape.gui.PoofSprite;
...
public class EnemyEntity extends Entity {
	...
	//	animTime++;				//ANIMAÇAO
	//	sprite.tex = defaultTex + animTime / 10 % 2;  //ANIMACAO
		move();
		if (xa == 0 || za == 0) {
...
//		double angle = Math.atan2(Player.zplayer-z, Player.xplayer-x);
	...
	//	z+=Math.sin(angle)*0.01;
		...
		//SISTEMA COLISAO BASEADO EM DISTANCIA
		double distance = Math.sqrt((Player.xplayer - x)* (Player.xplayer -x) + (Player.zplayer- this.z) * (Player.xplayer - this.z));
		if(distance < 1) {
	...
	protected void hurt(double xd, double zd) {
		...
		if (health <= 0) {
			int xt = (int) (x + 0.5);
			int zt = (int) (z + 0.5);
			level.getBlock(xt, zt).addSprite(new PoofSprite(x - xt, 0, z - zt)); //EXPL
			die();
			remove();
			Sound.kill.play();
		}
	...
	protected void collide(Entity entity) {
		...
		/*	Bullet bullet = (Bullet) entity;
			...
			*/
			this.remove();
			entity.remove();
			System.out.println("colisao com bullet");
